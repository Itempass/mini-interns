...
async def process_message(msg, message_id: str):
    ...
    for workflow in workflows:
        ...
        # The raw_data is the pydantic model of the email, the output processor
        # will handle creating the schema for it automatically.
        initial_workflow_data = InitialWorkflowData(
            raw_data=email_message,
            summary=f"New email from {msg.from_} - {msg.subject}",
        ...
        try:
            instance = await workflow_client.create_instance(
                ...
                triggering_data=initial_workflow_data,
                user_id=user_id,
            )
